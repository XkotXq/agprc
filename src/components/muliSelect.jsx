		...
		<div ref={wrapperRef} className="flex flex-col gap-2 relative">
			<div className="relative">
				<input
					ref={inputRef}
					className={cn(
						"file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-neutral-900 border-input h-9 w-full min-w-0 rounded-xl border bg-neutral-100 px-3 py-1 text-base shadow-xs transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
			...
			{isFocused && (
				<div className="border rounded-xl max-h-60 overflow-y-auto bg-neutral-100 absolute top-[calc(100%+8px)] shadow-2xl left-0 right-0 z-50 divide-y scrollCust jobList overflow-hidden">
					{filtered.length === 0 && (
					...
					{filtered.map((item, index) => {
						...
						return (
							<div
								key={item}
								className={`flex items-center gap-2 p-2 cursor-pointer hover:bg-neutral-200 ${
									highlightIndex === index ? "bg-neutral-300" : ""
								}`}
								...
								/>
								<span
									className={`w-5 h-5 rounded-xl border border-neutral-300 shrink-0 flex items-center justify-center  peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-neutral-200
